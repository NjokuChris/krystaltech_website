...
const CATEGORIES: Category[] = ["All", "Website", "Mobile App", "Branding", "Security"];
      ...
      <section className="px-5 pb-14 pt-16 md:px-10 md:pb-20 md:pt-24">
        <div className="mx-auto max-w-[1400px]">
          ...
          <motion.p {...fadeUp} transition={{ duration: 0.6, delay: 0.1 }}
            ...
            Here is a sample of websites, apps, brands, and security work we have shipped.
          </motion.p>

          <motion.div {...fadeUp} transition={{ duration: 0.6, delay: 0.15 }} className="mt-10 flex flex-wrap gap-10">
            {STATS.map(s => (
              <div key={s.label}>
          ...
          <motion.div {...fadeUp} transition={{ duration: 0.6, delay: 0.2 }} className="mt-10 flex flex-wrap gap-3">
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setFilter(cat)}
                className={`rounded-full px-5 py-2 text-sm font-medium transition-colors ${
                  filter === cat ? "bg-[#11142B] text-white" : "border border-[#11142B]/20 text-[#11142B] hover:bg-[#11142B]/5"
                }`}>
                {cat}
              </button>
            ))}
      ...
      <section className="px-5 py-10 md:px-10 md:py-16">
        <div className="mx-auto max-w-[1400px]">
          {filtered.length > 0 ? (
            ...
            <p className="py-16 text-center text-[#11142B]/40">
              {projects.length === 0 ? "Loading…" : "No projects in this category yet."}
            </p>
          )}
        </div>
      </section>
